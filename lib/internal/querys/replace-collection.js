  ...
  return parley(
    ...
    function (done){
      ...
      _self.tenancy(req, ignore_tenancy).then(model => {
        model.datastore = req ? req.headers['x-tenant-id'] : 'default';
        try {
        ...
        (function (proceed){
          ...
          if (manyToMany) {
            ...
            //  ╩╚═╚═╝╝╚╝  ─┴┘└─┘└─┘ ┴ ┴└─└─┘ ┴   └─┘└└─┘└─┘┴└─ ┴
            if (req) {
              WLChild.destroy(req, criteriaOfDestruction, function $afterDestroyingChildRecords(err) {
          ...
          (function(proceed){
            ...
            if (req) {
              WLChild.count(req, potentialCollisionCriteria, function(err, total) {
                if (err) { return proceed(err); }
            ...
            if (query.associatedIds.length > 0) {
              ...
              if (req) {
                WLChild.update(req, partialNullOutCriteria, partialNullOutVts, function(err) {
                  if (err) { return proceed(err); }
            ...
            // Alternatively, we'll go with scenario B, where we potentially null all the fks out.
              ...
              if (req) {
                WLChild.update(req, nullOutCriteria, blanketNullOutVts, function(err) {
                  if (err) { return proceed(err); }
                  return proceed();
