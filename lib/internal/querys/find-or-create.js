...
module.exports = function findOrCreate( /* criteria?, newRecord?, explicitCbMaybe?, meta? */ ) {
  ...
  if (typeof args[0] === 'string' || typeof args[0] === 'object') {
    if(typeof args[0] === 'string' && datasources.searchStringIdentity(args[0])) {
      ...
      || args[0].constructor.name === 'DataStoreConfig')) {
      req = args[0];
      args.splice(0,1);
      ignore_tenancy = false;
    }
  ...
  return parley(
    ...
    function (done){
      ...
      _self.tenancy(req, ignore_tenancy).then(model => {
        ...
        if (req) {
          WLModel.findOne(req, query.criteria, function _afterPotentiallyFinding(err, foundRecord) {
            if (err) {
              return done(err);
            }
            ...
            //  ║╣ ╔╩╦╝║╣ ║  ║ ║ ║ ║╣   │  ├┬┘├┤ ├─┤ │ ├┤   │─┼┐│ │├┤ ├┬┘└┬┘
            //  ╚═╝╩ ╚═╚═╝╚═╝╚═╝ ╩ ╚═╝  └─┘┴└─└─┘┴ ┴ ┴ └─┘  └─┘└└─┘└─┘┴└─ ┴
            WLModel.create(req, query.newRecord, function _afterCreating(err, createdRecord) {
              if (err) {
                return done(err);
              }
