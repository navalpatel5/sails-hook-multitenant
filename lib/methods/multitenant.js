...
module.exports = function multitenant(req, model, self, sails, schema, WLModel) {
    ...
    return new Promise(async (resolve,reject) => {
        ...
        if (req && req.constructor.name === 'IncomingMessage') {
            ...
            if (!config) {
                resolve();
            } else if (_.has(config, 'identity') ? _datasource.search(config.identity) : undefined) {
                ...
                if(d.isDatasource(config) && config.identity) {
                    ...
                    await createDatasource(model, _newdatasource, config.identity, schema, function (error) {
                        if(error){
                            sails.log.error('[sails-hook-multitenant/lib/methods/multitenant] [ERROR] => ', error);
                            sails.log.error('[sails-hook-multitenant/lib/methods/multitenant] [CONFIG] => ', config);
                            resolve();
                        }else{
                            _self.datastore = config.identity;
                            resolve(_self);
